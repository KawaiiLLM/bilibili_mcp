...
export function extractBVId(input: string): string {
  ...
  if (!match) throw new Error("Invalid Bilibili video ID or URL");
  return match[0];
}
...
export function extractAid(input: string): number {
  const match = input.match(AID_PATTERN);
  const aid = match ? Number(match[1]) : Number.NaN;
  if (!Number.isFinite(aid) || aid <= 0) throw new Error("Invalid Bilibili AV ID or URL");
  return Math.floor(aid);
}

export function containsAid(input: string): boolean {
  return AID_PATTERN.test(input);
}

export function isValidBVId(input: string): boolean {
  return /^BV[A-Za-z0-9]{10}$/.test(input);
}

export function isValidAid(input: string): boolean {
...
export async function resolveBilibiliVideoInput(input: string): Promise<string> {
  ...
  if (containsBVId(cleaned)) return cleaned;
  ...
  if (!SHORT_HOSTS.has(host) && !VIDEO_HOSTS.has(host)) return cleaned;
  if (!SHORT_HOSTS.has(host)) return url;

  const redirected = await fetchRedirectedUrl(url);
  if (containsBVId(redirected)) return redirected;
