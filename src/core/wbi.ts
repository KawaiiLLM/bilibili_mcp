...
export async function withWbiSignature(
...
async function getWbiKeys(signal?: AbortSignal): Promise<{ mixKey: string }> {
  const now = Date.now();
...
function extractKey(url: unknown): string | undefined {
