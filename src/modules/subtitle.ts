  ...
  if (subtitles.length === 0) {
    ...
    if (!login.isLogin) {
      throw new BilibiliAPIError(
        ...
        undefined,
        true,
        "请确认 CookieCloud 中的 B 站登录态仍然有效，并重新同步 Cookie。",
      );
    }
  }
...
export function selectBestSubtitle(subtitles: any[], preferredLang?: string): any | undefined {
  if (!Array.isArray(subtitles) || subtitles.length === 0) return undefined;
  const langs = [preferredLang, "zh-Hans", "zh-CN", "zh-Hant", "en"].filter(Boolean) as string[];
