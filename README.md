...
| `quality` | number | 否 | 清晰度 qn,`stream` 使用 |
| `timestamp` | number | 否 | 目标时间戳/秒,`snapshot` 使用 |
...
```json
{ "action": "snapshot", "input": "BV19v411r76g", "page": 1, "timestamp": 60 }
```

#### action: `stream`

获取视频流地址(音视频 URL)。需要登录态获取高清晰度。

```json
{ "action": "stream", "input": "BV19v411r76g", "page": 1, "quality": 80 }
```
