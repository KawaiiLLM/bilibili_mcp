...
/plugin install bilibili-mcp@claude-beats
```

之后 `/reload-plugins` 即可使用。需要登录的能力先调一次 `bilibili_config setup` 配 CookieCloud(见下方[CookieCloud 配置](#cookiecloud-配置))。

更新到新版本:`/cc-fresh:check` 查看 → `/plugin` 菜单里禁用再启用 `bilibili-mcp`(GitHub source 插件需要走 UI 重新拉)。

### 本地源码运行

```bash
git clone https://github.com/KawaiiLLM/bilibili_mcp.git
cd bilibili_mcp
npm install
...
```bash
BILIBILI_MCP_HTTP_PORT=3000 node dist/cli.js http
```
...
```json
{
  "mcpServers": {
    "bilibili-mcp": {
      "command": "node",
      "args": ["/absolute/path/to/bilibili_mcp/dist/cli.js", "stdio"]
    }
  }
}
```

HTTP 模式默认端点 `/mcp`、`/sse`、`/messages`。
