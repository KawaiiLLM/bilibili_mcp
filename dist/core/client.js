...
export async function request(endpoint, params = {}, ctx = {}) {
    return performWithAuthRefresh(endpoint, params, ctx, false);
}
async function performWithAuthRefresh(endpoint, params, ctx, forceRefresh) {
...
async function performRequest(endpoint, params, ctx, forceRefresh) {
    ...
    if (endpoint.buvid) {
        ...
        if (buvid)
            headers.Cookie = appendBuvidCookies(headers.Cookie, buvid);
    }
    ...
    if (endpoint.wbi2)
        requestParams = addWbi2Params(requestParams);
    if (endpoint.wbi) {
        const signed = await withWbiSignature(endpoint.params_type === "query" ? requestParams : {}, ctx.signal);
        if (endpoint.params_type === "query")
            requestParams = signed;
        else
            appendQueryParams(url, signed);
    }
    const init = { method: endpoint.method, headers, signal: ctx.signal };
    ...
    const result = await withRetry(async () => {
        ...
        return parseResponse(endpoint, response, url.toString());
    }, DEFAULT_RETRY_OPTIONS);
    if (cacheKey)
        cacheManager.set(cacheKey, result, getCacheTtlMs(endpoint));
    return result;
}
...
function buildUrl(endpoint, params, pathParams) {
    const replaced = endpoint.url.replace(/\{([A-Za-z0-9_]+)\}/g, (_match, name) => {
...
function buildCacheKey(endpoint, url, params, credential, ctx) {
...
function getCacheTtlMs(endpoint) {
    if (endpoint.url.includes("/x/player/videoshot"))
        return 24 * 60 * 60 * 1000;
    if (endpoint.url.includes("/x/v2/reply/"))
        return 30 * 60 * 1000;
    return 60 * 60 * 1000;
}
...
async function resolveCredential(endpoint, ctx, forceRefresh) {
    if (!endpoint.auth && !endpoint.csrf)
...
async function parseResponse(endpoint, response, url) {
    ...
    if (typeof payload.code === "number" && payload.code !== 0)
        throw mapBilibiliError(payload, url);
    return (payload.data ?? payload.result ?? payload);
}
function mapBilibiliError(payload, url) {
...
function appendBiliTicket(cookieHeader, ticket, expireAt) {
...
function isWbiRecoverable(error) {
    if (!(error instanceof BilibiliAPIError))
        return false;
    if (error.code === "BILIBILI_WBI_FAILED")
        return true;
    if (error.code !== "BILIBILI_AUTH_REQUIRED")
        return false;
    ...
    const payloadCode = error.originalError?.code;
