...
export function murmur3x64_128(input, seed) {
    ...
    if (remaining >= 15)
        k2 ^= BigInt(bytes[offset + 14]) << 48n;
    if (remaining >= 14)
        k2 ^= BigInt(bytes[offset + 13]) << 40n;
    if (remaining >= 13)
        k2 ^= BigInt(bytes[offset + 12]) << 32n;
    if (remaining >= 12)
        k2 ^= BigInt(bytes[offset + 11]) << 24n;
    if (remaining >= 11)
        k2 ^= BigInt(bytes[offset + 10]) << 16n;
    if (remaining >= 10)
        k2 ^= BigInt(bytes[offset + 9]) << 8n;
    if (remaining >= 9) {
    ...
    if (remaining >= 8)
        k1 ^= BigInt(bytes[offset + 7]) << 56n;
    if (remaining >= 7)
        k1 ^= BigInt(bytes[offset + 6]) << 48n;
    if (remaining >= 6)
        k1 ^= BigInt(bytes[offset + 5]) << 40n;
    if (remaining >= 5)
        k1 ^= BigInt(bytes[offset + 4]) << 32n;
    if (remaining >= 4)
        k1 ^= BigInt(bytes[offset + 3]) << 24n;
    if (remaining >= 3)
        k1 ^= BigInt(bytes[offset + 2]) << 16n;
    if (remaining >= 2)
        k1 ^= BigInt(bytes[offset + 1]) << 8n;
    if (remaining >= 1) {
...
function activationContent(uuid) {
    return {
        "3064": 1,
        "5062": Date.now(),
...
export function buildActivationPayload(uuid) {
